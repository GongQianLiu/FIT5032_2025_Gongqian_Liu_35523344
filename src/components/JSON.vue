      ...
      <ul>
        <li v-for="author in authors" :key="author.id">
          <span
            ...
            :style="author.name === 'George Orwell' ? { fontWeight: 'bold', color: '#fff', backgroundColor: '#42b883', padding: '2px 8px', borderRadius: '4px' } : {}"
            :title="author.name === 'George Orwell' ? 'Featured Author' : author.name"
            :data-author-type="author.name === 'George Orwell' ? 'featured' : 'regular'"
          >
            {{ author.name }}
          </span>
          ({{ author.birthYear }})
